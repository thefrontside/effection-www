...
export function startServer() {
  return resource<ServerInfo>(function*(provide) {
    ...
    let library = yield* LibraryRepositoryContext.set(
      yield* loadRepository({
        owner: "thefrontside",
        name: "effection",
      }),
    );

    let contrib = yield* ContribRepositoryContext.set(
      yield* loadRepository({
        owner: "thefrontside",
    ...
    yield* provide(yield* revolution.start());
  });
}